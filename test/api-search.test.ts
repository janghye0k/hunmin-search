...
describe('searchHangulRanked', () => {
  ...
  it('includeNonMatching appends failures with score 0', () => {
    const out = searchHangulRanked('xyz', ['abc', 'xayz'], { includeNonMatching: true });
...
describe('searchHangulDetailed', () => {
  ...
  it('omits editTrace by default', () => {
    const out = searchHangulDetailed('a', ['a']);
    expect(out[0]!.editTrace).toBeUndefined();
  });

  it('includes editTrace when includeEditTrace is true', () => {
    const out = searchHangulDetailed('a', ['a'], { includeEditTrace: true });
  ...
  it('does not compute editTrace for subsequence failures', () => {
    const out = searchHangulDetailed('zzz', ['abc'], { includeNonMatching: true, includeEditTrace: true });
    expect(out).toHaveLength(1);
